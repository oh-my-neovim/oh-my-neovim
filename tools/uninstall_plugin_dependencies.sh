...
if [ "$confirmation" = y ] || [ "$confirmation" = Y ]; then
  ...
  if [ "$(uname)" = Darwin ]; then
    ...
    for plugin in $plugins; do
      if [ -f $OH_MY_NEOVIM/templates/$plugin/brew ]; then
        ...
        printf "${BLUE}Remove $brew_packages with brew for $plugin ...${NORMAL}\n"
  ...
  if [ "$OS_TYPE" = Linux ]; then
    ...
    if [ -f /etc/debian_version ]; then
      if [ -f $OH_MY_NEOVIM/templates/$plugin/apt ]; then
        ...
        printf "${BLUE}Remove $apt_packages with apt for $plugin ...${NORMAL}\n"
      ...
      if [ -f $OH_MY_NEOVIM/templates/$plugin/yum ]; then
        ...
        printf "${BLUE}Remove $yum_packages with apt for $plugin ...${NORMAL}\n"
        printf "${RED}sudo permissions required${NORMAL}\n"
  ...
  for plugin in $plugins; do
    ...
    if [ -f $OH_MY_NEOVIM/templates/$plugin/uninstall.sh ]; then
      printf "${BLUE}Remove dependencies for $plugin ...${NORMAL}\n"
    ...
    if [ -f $OH_MY_NEOVIM/templates/$plugin/pip2 ]; then
      ...
      printf "${BLUE}Uninstall $pip2_packages for $plugin ...${NORMAL}\n"
    ...
    if [ -f $OH_MY_NEOVIM/templates/$plugin/pip3 ]; then
      ...
      printf "${BLUE}Uninstall $pip3_packages for $plugin ...${NORMAL}\n"
    ...
    if [ -f $OH_MY_NEOVIM/templates/$plugin/gem ]; then
      ...
      printf "${BLUE}Uninstall $gem_packages for $plugin ...${NORMAL}\n"
    ...
    if [ -f $OH_MY_NEOVIM/templates/$plugin/npm ]; then
      ...
      printf "${BLUE}Remove $npm_packages with node package manager for $plugin ...${NORMAL}\n"
